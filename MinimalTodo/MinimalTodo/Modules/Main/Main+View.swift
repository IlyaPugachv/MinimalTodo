...
extension Main {
    class View: UIViewController {
        ...
        private let todoListCellIdentifier = "TodoListCell"
        ...
        private func setupSearchView() {
            ...
            searchView.tableView.register(UITableViewCell.self, forCellReuseIdentifier: todoListCellIdentifier)
        ...
        private func setupActions() {
            ...
            searchImageView.addGestureRecognizer(searchTapGesture)
...
extension Main.View: UISearchBarDelegate, UITableViewDataSource, UITableViewDelegate {
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        filterTodoLists(for: searchText)
    }
    
    private func filterTodoLists(for query: String) {
        searchResults = todoLists.filter { $0.title.lowercased().contains(query.lowercased()) }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let todoList = searchResults[indexPath.row]
        let todoListView = TodoListView(todoList: todoList)
        todoListView.delegate = self
        todoListView.translatesAutoresizingMaskIntoConstraints = false
        
        cell.contentView.addSubview(todoListView)
        
        NSLayoutConstraint.activate([
            todoListView.leadingAnchor.constraint(equalTo: cell.contentView.leadingAnchor),
            todoListView.trailingAnchor.constraint(equalTo: cell.contentView.trailingAnchor),
            todoListView.topAnchor.constraint(equalTo: cell.contentView.topAnchor),
            todoListView.bottomAnchor.constraint(equalTo: cell.contentView.bottomAnchor),
            todoListView.heightAnchor.constraint(equalToConstant: 80),
        ])
